...
let input : string = `2911threeninesdvxvheightwobm
...
var inputArray: Array<string> = input.split("\n");

inputArray.forEach((currentString) => {
  let arrOfNumber: Array<number> = currentString.match(REGEX);
  let numbers: string = arrOfNumber.toString();  
  let firstNumber: string = numbers.slice(0, 1);
  let lastNumber: string = numbers.slice(numbers.length - 1, numbers.length);
  let completeNumber: number = parseInt(firstNumber + lastNumber);  
  total += completeNumber;
  console.log("String: " + currentString + ", number: " + arrOfNumber + ": " + firstNumber + " & " + lastNumber + " => " + completeNumber + " (total = " + total + ")");
});
...
appDiv.innerHTML = total.toString();
