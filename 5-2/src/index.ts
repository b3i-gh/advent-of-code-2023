...
var seeds : number[] = [];
// get the seeds:
seeds =  readLineNumbers(splittedInput[0]);
var explodedSeeds = explodeSeeds(seeds);
console.log("seeds:" + seeds);
console.log("exploded seeds:" + explodedSeeds);

var output : number = -1;

// get the seed-to-soil-map
var seedToSoilMap : number[][] = [];
...
var soilToFertilizerMap : number[][] = [];
...
var fertilizerToWarerMap : number[][] = [];
...
var lightToTemperatureMap : number[][] = [];
...
var temperatureToHumidityMap : number[][] = [];
...
var humidityToLocationMap : number[][] = [];
...
let transformedFinal = explodedSeeds;
...
function readLineNumbers(completeLine : string){
    let numbers : number[] = [];
...
function transform(sourceData : number[], transformationMap : number[][], finalTransformation : boolean = false){
...
function explodeSeeds(seeds : number[]){
    let explodedSeeds : number[] = [];
    for(let i = 0; i < seeds.length; i=i+2){
        let start = seeds[i];
        let lenght = seeds[i+1];
        
        for(let j = start; j < start+lenght; j++)
        explodedSeeds.push(j);
    }
    return explodedSeeds;
}