...
spilttedInput.forEach((row) => {
  ...
  rowHistory[rowHistory.length - 1].unshift(0);
...
function calculateNext(history: number[][]) {
  ...
  for (let i = history.length - 1; i > 0; i--) {
    ...
    let rowNumber = currRow[0];
    let upRowNumber = prevRow[0];
    let nextNumber = upRowNumber - rowNumber;
    history[i - 1].unshift(nextNumber);
